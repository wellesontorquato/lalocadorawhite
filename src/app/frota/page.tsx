...
export default function PaginaFrotaDetalhe({
  params,
}: {
  params: { id: string };
}) {
  ...
  const contatoHref = useMemo(() => {
    ...
    // Se vier do Hero, já manda junto pro /contato
        ...
        <section className="max-w-[1100px] mx-auto px-6 md:px-12 py-16">
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-8">
            ...
            <p className="mt-2 text-slate-600">
              Esse ID não existe na sua constante <code className="px-1 py-0.5 bg-white border border-slate-200 rounded">FROTA</code>.
            </p>

            <Link
              href="/frota"
              className="mt-6 inline-flex items-center gap-2 rounded-full px-6 py-3 bg-slate-900 text-white text-[11px] font-black uppercase tracking-widest hover:bg-brand-blue hover:text-slate-900 transition-all"
            >
              <ChevronLeft size={18} />
              Voltar para Frota
            </Link>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 md:p-8 shadow-[0_24px_80px_-60px_rgba(2,6,23,0.35)]">
              ...
              <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="rounded-2xl bg-slate-50 border border-slate-200 p-4">
                  ...
                  <p className="mt-1 text-3xl font-black tracking-tight leading-none text-slate-900">
              ...
              <p className="mt-3 text-[11px] text-slate-500 font-light leading-relaxed">
                *Confirmação final de disponibilidade e itens pelo WhatsApp.
              </p>
            </div>
          </motion.div>
        </div>
          ...
          {carro.features?.length ? (
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-8">
              <p className="text-slate-600">
                *Ainda não existem features cadastradas para esse veículo.
              </p>
            </div>
          )}
        </div>
